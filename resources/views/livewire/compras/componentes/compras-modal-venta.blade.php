...
<div x-data="{ show: @entangle('showConfirmModal') }" x-show="show" x-transition class="fixed inset-0 z-50 flex items-center justify-center"
    ...
    <div
        ...
        <div class="flex gap-4">
            @if (session('compras_stream_pdf_url'))
                <a href="{{ session('compras_stream_pdf_url') }}" target="_blank"
                    class="px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700">Ver
                    PDF</a>
            @endif
            @if (session('compras_download_pdf_url'))
                <a href="{{ session('compras_download_pdf_url') }}" target="_blank"
                    class="px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700">Descargar PDF</a>
            @endif
