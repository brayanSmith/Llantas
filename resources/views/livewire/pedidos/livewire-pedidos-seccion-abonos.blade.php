                ...
                <tbody class="bg-white dark:bg-gray-800/50">
                    <template x-for="(abono, index) in pedido.abonos" :key="index">
                        <tr class="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                ...
                                <template x-if="abono.imagen">
                                    <div class="flex justify-center">
                                        <a :href="abono.imagen.startsWith('http') ? abono.imagen : '/storage/' + abono.imagen" target="_blank" class="group relative inline-block">
                                            <img
                                                :src="abono.imagen.startsWith('http') ? abono.imagen : '/storage/' + abono.imagen"
                                                alt="Comprobante"
                            ...
                            <!-- Observaciones -->
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-600 dark:text-gray-400" x-text="abono.descripcion || '-'"></span>
                            </td>

                            <!-- Monto -->
                            <td class="px-6 py-4">
                                <span class="text-sm font-bold text-green-600 dark:text-green-400 text-right block" x-text="Number(abono.monto).toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 2 })"></span>
                            </td>


                            <!-- Fecha -->
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-700 dark:text-gray-300 font-medium" x-text="new Date(abono.fecha).toLocaleDateString('es-CO', { year: 'numeric', month: 'short', day: 'numeric' })"></span>
                            </td>

                            <!-- Acciones -->
                            <td class="px-6 py-4 text-center">
                                <div class="flex gap-2 justify-center">
                                    <button type="button" @click="selectAbono(abono)" class="text-blue-600 hover:text-blue-800 dark:hover:text-blue-400">
