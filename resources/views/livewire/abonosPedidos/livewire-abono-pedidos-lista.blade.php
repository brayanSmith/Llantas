    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                <tr>
                    <th class="px-3 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Codigo</th>
                    <th class="px-3 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Fecha</th>
            ...
            <tbody>
                <template x-for="(pedido, index) in pedidosEnCartera" :key="pedido.id">
                    <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors duration-150"
                        ...
                        <td class="px-3 py-3 text-sm text-gray-900 dark:text-gray-100 font-medium">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200"
                                  x-text="pedido.codigo"></span>
                        </td>

                        <!-- Fecha -->
                        <td class="px-3 py-3 text-sm text-gray-900 dark:text-gray-100" x-text="pedido.fecha"></td>

                        <!-- Total a Pagar -->
                        <td class="px-3 py-3 text-sm text-right font-semibold text-gray-900 dark:text-gray-100">
                            <span x-text="parseFloat(pedido.total_a_pagar).toLocaleString('es-CO', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                        </td>

                        <!-- Saldo Pendiente -->
                        <td class="px-3 py-3 text-sm text-right font-semibold"
                            :class="pedido.saldo_pendiente > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400'">
                            <span x-text="parseFloat(pedido.saldo_pendiente).toLocaleString('es-CO', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                        </td>

                        <!-- Checkbox -->
                        <td class="px-3 py-3 text-center">
                            <input type="checkbox"
                                   :checked="pedidosSeleccionados.some(p => p.id === pedido.id)"
                                   @change="togglePedido(pedido)"
        ...
        <template x-if="pedidosEnCartera.length === 0">
            <div class="w-full px-4 py-8 text-center bg-white dark:bg-gray-800">
                ...
                <p class="text-gray-500 dark:text-gray-400 text-sm">Seleccione un proveedor para ver los pedidos en cartera</p>
            </div>
        </template>
    </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-900 px-4 py-2">
            <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">
                    Total de pedidos: <span class="font-semibold text-gray-900 dark:text-white" x-text="pedidosEnCartera.length"></span>
                </span>
                <span class="text-gray-600 dark:text-gray-400">
                    Seleccionadas: <span class="font-semibold text-blue-600 dark:text-blue-400" x-text="cantidadPedidosSeleccionados"></span>
                </span>
            </div>
        </div>
    </template>
